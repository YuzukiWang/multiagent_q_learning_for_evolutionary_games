...
for i in range(2):
    for j in range(2):
        if i==0 and j==0:
            ...
            data_filename = "WM_cooperarion_rate.npy"
            text_label = 'Well-mixed population'
            final_result = np.load(data_filename)
        elif i==1 and j==0:
            data_filename = "lattice_cooperation_rate.npy"
            text_label = 'Lattice'
            final_result = np.load(data_filename)
        elif i==1 and j==1:
            data_filename = "SF_cooperation_rate.npy"
        ...
        for spine in main_axes.spines.values():
            spine.set_linewidth(0.5)
        img = main_axes.imshow(final_result, cmap='YlGnBu_r', interpolation='nearest', vmin=0.7, vmax=1)
        ...
        fig.text(axs[i, j].get_position().x0 + axs[i, j].get_position().width / 2, axs[i, j].get_position().y0 + axs[i, j].get_position().height * 0.1, text_label, fontsize=12, ha='center', va='bottom')
...
cbar = fig.colorbar(img, cax=cax, shrink=0.72, ticks=[0.7, 1.0])
cbar.ax.tick_params(axis='y', length=0, width=0.5)
cbar.set_label('Cooperation rate', rotation=90, fontsize=12, labelpad=-12)
