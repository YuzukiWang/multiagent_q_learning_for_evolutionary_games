...
population_size = 0 # How many AIs in the population此处改为0即为没有智能体
mentor_instances = 5 # How many instances of each defined strategy there are
# mentor_types = 7 #测试训练集
mentor_strategy = [0,2,3,7,8,9,10,13,15]
# mentor_strategy = [0,2,3,4,15]
...
class AgentDefined:
    ...
    def typeagent(self):
        if self.strategy == 0:#"TFT"
            ...
            return 16
        elif self.strategy == 16:#"CURE"
            return 17
        elif self.strategy == 17:#"AON2"
            return 18
        elif self.strategy == 18:#"ZDSet2"
            return 19
...
strategy0729payoffmatrix = np.load('PAYOFFMATRIX_0412/strategy0729payoffmatrix+zd+ALLD.npy') #导入策略两两交互的结果


# Testing mode

max_episode = 100
avg_num_strategy= [ [ 0.0 for _ in range(20)  ] for _ in range(max_episode) ] #统计策略的变化
...
i_num_strategy = [ [ [ 0 for _ in range(20)  ] for _ in range(max_episode) ] for _ in range(repeattimes) ]
...
for repeatnum in range(repeattimes):
    ...
    num_strategy = [ [ 0 for _ in range(20)  ] for _ in range(max_episode) ] #统计策略的变化
    ...
    for iepisode in range(max_episode):
        ...
        #求所有人的平均收益
        avg_reward_total[iepisode] = sum(avg_reward) / (len(bigpopulation))
        ...
        for i_num in range(len(bigpopulation)) :
            ...
            elif type_player[i_num] == 16 :
                num_strategy[ iepisode][16] +=1
            elif type_player[i_num] == 17 :
                num_strategy[ iepisode][17] +=1
            elif type_player[i_num] == 18 :
                num_strategy[ iepisode][18] +=1
            elif type_player[i_num] == 19 :
                num_strategy[ iepisode][19] +=1
         
        episode_total += 1
        
        ifbreak = 0 #检查是否已经演化完毕
        for check_break in range(20):  #最多可能有16个策略
        ...
        if ifbreak == 1024 or  episode_total == max_episode:
            ...
            avg_reward_globle += avg_reward_total/repeattimes
...
for j in range(20):#最多可能有16个策略
    # y[j]=[  i[j] for i in num_strategy ]     
    y[j]=[  ( avg_num_strategy[i][j] /  ( mentor_instances * (len(mentor_strategy) + len(population)/mentor_instances ) ) )  for i in range(max_episode) ] 
# fig =plt.figure(1)#绘图

file_path = "special/noMTBRnoGTFT.npy"
np.save(file_path, y)
...
for irepeat in range(repeattimes):
    for jrepeat in range(20):
        yi[irepeat][jrepeat] = [  ( i_num_strategy[irepeat][i][jrepeat] /  ( mentor_instances * (len(mentor_strategy) + len(population)/mentor_instances) ) )  for i in range(max_episode) ]
    x=[value * 224 for value in range(max_episode)]
...
file_path = "special/noMTBRnoGTFT_多次.npy"
