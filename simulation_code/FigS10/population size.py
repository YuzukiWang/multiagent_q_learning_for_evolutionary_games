...
N_list = [15,30,45,60,75,90,120,150,180,225,270,315,360,450,540] #选择强度
s = 1
...
def main(N):
    ...
    population_size=int(N/15)
    mentor_instances=int(N/15)
    ...
    strategy0729payoffmatrix = np.load('strategy0729payoffmatrix.npy') #导入策略两两交互的结果
    ...
    for repeatnum in range(repeattimes):
        ...
        for iepisode in range(max_episode):
            ...
            if ifbreak == 1024 or  episode_total == max_episode:
                ...
                avg_episode_total+=episode_total/repeattimes
                if num_strategy[episode_total-1][0]>0.5*mentor_instances * (len(mentor_strategy) + 1):
                    prob_BRTMwin += 1/repeattimes
                break;
                
    return(avg_episode_total,prob_BRTMwin)

if __name__=="__main__":
    result=list(pool.map(main,N_list))
    end = time.time()
    print(end-start) 
...
plt.rcParams['font.family'] = 'Times New Roman'
np.save("figS2_种群规模/种群规模.npy", result)
...
# file_path = "fig2_普通演化/无BRTM_多次.npy"
...
# ax_legend.axis('off')
# legend_legend1 = ax_legend.legend(handles=legend_main.legendHandles[:7], labels=[text.get_text() for text in legend_main.get_texts()][:7], loc='center', fontsize='x-large')
...
# y3=np.load('fig2_普通演化/globle_avg_payoff_without_BRTM.npy')
# for i_print in range(len(bigpopulation)):
#     print (   bigpopulation[i_print].strategy_print()     )

# fig2,ax = plt.subplots(figsize=(6, 6))
# ax.tick_params(axis='both', direction='in')
# fig2.suptitle('Globle Average Payoff during Evolutionary Process',fontsize=18)
# x=[value * 224 for value in range(max_episode)]
# y2=avg_reward_globle
# # np.save('fig2_普通演化/globle_avg_payoff_with_BRTM.npy',avg_reward_globle)
# plt.plot(x,y2,linewidth=1.0, linestyle="-",color='red',label="population")
# plt.plot(x,y3,linewidth=1.0, linestyle="-",color='blue',label="population without BRTM")
# plt.ylim(1,3)
# plt.xticks(fontsize=14)
# plt.yticks(fontsize=14)
# plt.legend(loc="best")
# plt.xlabel('Generations',fontsize=18)
# plt.ylabel('Globle Average Payoff',fontsize=18)
# plt.show
