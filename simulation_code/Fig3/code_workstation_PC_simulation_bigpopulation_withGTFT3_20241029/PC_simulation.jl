...
function main(mode, delta, beginning_index, ending_index)
    ...
    if mode == 1
    PayoffMatrix = PM
    outfile = "WithMTBR_delta$(delta)_$(index).txt"
    elseif mode ==2
    PayoffMatrix = PM[2:end,2:end]
    outfile = "WithoutMTBR_delta$(delta)_$(index).txt"
    end
...
if abspath(PROGRAM_FILE) == @__FILE__
    if length(ARGS) < 4
        ...
        main(mode, delta, beginning_index, ending_index)
    end
end
