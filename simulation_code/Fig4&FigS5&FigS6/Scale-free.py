...
def main(S,T):   
    ...
    N=100
    graph=nx.random_graphs.barabasi_albert_graph(N,2)
    ...
    number_record,neibour_record=f_node_neibour(graph, N) #记录邻居
    ...
    for repeatnum in range(repeattimes):
        ...
        bigpopulation = random.choices(population+mentors, k = N)
        ...
        for iepisode in range(max_episode):
            ...
            for check_break in range(16):  #最多可能有16个策略
                if num_strategy[ iepisode][check_break] >= N :
                    for iall in range(iepisode+1,max_episode):
                        
                        num_strategy[ iall][check_break] = N 
...
if __name__=="__main__":
    ...
    np.save("fig3.1_复杂网络+收益矩阵/策略演化_SF_合作率.npy", final_result)
    np.save("fig3.2_复杂网络+收益矩阵/策略演化_SF_平均收益.npy", final_result2)
...
#%%
    ...
    file_path = "fig3_复杂网络+收益矩阵/策略演化_SF.npy"
    ...
    # final_result_draw=np.flip ( np.array(result_f0729) / 100   , axis=1)
