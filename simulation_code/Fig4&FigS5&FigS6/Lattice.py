...
population_size = 14 # How many AIs in the population此处改为0即为没有智能体
...
def main(S,T):   
    ...
    graph=nx.grid_2d_graph(m,n,periodic=False) #无周期性
    print(graph.edges)
    
    
    def f_node_neibour(graph,N):  #生成邻居并统计度 
        number_record=np.zeros(N,dtype=int)
        neibour_record=np.zeros((N,N),dtype=int)-1
        int_element_pairs = [((x1 * m + y1), (x2 * m + y2)) for ((x1, y1), (x2, y2)) in graph.edges()]
        for col_label,row_label in int_element_pairs:
        ...
        return number_record,neibour_record
    
    number_record,neibour_record=f_node_neibour(graph, m * n) #记录邻居
    
    
    
    # Testing mode
    
    max_episode = 100 #每轮的最大回合数
    ...
    repeattimes = 100 #重复试验次数
    
    for repeatnum in range(repeattimes):
        
        # bigpopulation=population+mentors
        bigpopulation = random.choices(population+mentors, k = m * n)
        ...
        for iepisode in range(max_episode):
            ...
            for iplayer1 in range(len(bigpopulation)):  #每个人都和邻居博弈
                
                for iplayer2 in neibour_record[ iplayer1 , 0:number_record[iplayer1] ] :
            ...
            for iavg_reward in range(len(bigpopulation)) : #求平均收益+合作率
                avg_reward[iavg_reward] = sum(reward_episode[iavg_reward]) / number_record[iavg_reward]
                avg_cooprate[iavg_reward] = sum(cooprate_episode[iavg_reward]) / number_record[iavg_reward]
            ...
            for irefresh in range(len(bigpopulation)): #更新
                ...
                opponent_refresh = random.choice(neibour_record[irefresh, 0:number_record[irefresh]])
            ...
            for check_break in range(16):  #最多可能有16个策略
                if num_strategy[ iepisode][check_break] >= m * n :
                    for iall in range(iepisode+1,max_episode):
                        
                        num_strategy[ iall][check_break] = m * n 
    ...
    return (avg_cooprate_globle[-1]), (avg_reward_globle[-1]), (avg_num_strategy[-1][0])
...
if __name__=="__main__":
    ...
    final_result3=np.flip ( np.array(final_result3)    , axis=1)
    # file_path = "fig3_复杂网络+收益矩阵/策略演化_WM.npy"
    # np.save("fig3.1_复杂网络+收益矩阵/策略演化_lattice_合作率.npy", final_result)
    # np.save("fig3.2_复杂网络+收益矩阵/策略演化_lattice_平均收益.npy", final_result2)
    # np.save("fig3_复杂网络+收益矩阵/策略演化_lattice.npy", final_result3)
    
    final_result_draw=final_result
#%%
    plt.rcParams['font.family'] = 'Times New Roman'
    
    file_path = "fig3_复杂网络+收益矩阵/策略演化_lattice.npy"
    # np.save(file_path, final_result_draw)
    ...
    img = main_axes.imshow(final_result_draw, cmap='YlGnBu_r', interpolation='nearest', vmin=1, vmax=0)
    ...
    # x_labels = [1.5, 2.5, 3.5, 4.5]
    # y_labels = [2.5, 1.5, 0.5, -0.5]
    # plt.figure(figsize=(4, 4))
    # plt.xticks(np.arange(len(x_labels)), x_labels)
    # plt.yticks(np.arange(len(y_labels)), y_labels)
    # plt.imshow(final_result_draw, cmap='viridis', interpolation='nearest', vmin=0, vmax=1)
    # plt.colorbar()
    # plt.show()
